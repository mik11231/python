...
def partitions(n: int, k: int) -> list[tuple[int, ...]]:
    """All ways to write n as sum of k non-negative integers (order matters)."""
    if k == 1:
        return [(n,)]
    out = []
    for x in range(n + 1):
...
def solve(s: str) -> int:
    """Max score with 100 tsp total. Score = product of max(0, cap), max(0,dur), flavor, texture (no calories)."""
    ingredients = parse(s)
    if not ingredients:
        return 0
    best = 0
